...
class Window:
    ...
    def MainLoop(self):
        self.root.title("Cleaner by Dhruv")
        self.root.minsize(500, 400)
        self.root.maxsize(500, 400)
        self.root.mainloop()

    def MenuAbout(self):
        tkinter.messagebox.showinfo("Cleaner by Dhruv",
                                    "This a disk clean-up program, made by Dhruv." +
                                    "Contact me at Discord- Dhruv Jadav#1620." +
						"Contact by email- dhruv.y.jadav@gmail.com" +
						"Contact at Instragram- dhruv.jadav.1417")

    def MenuExit(self):
        self.root.quit()

    def MenuScanRubbish(self):
        result = tkinter.messagebox.askquestion("Cleaner by Dhruv", "Scaning" +
                                                " junk files may cost much time. Continue?")
        if result == 'no':
            return
        tkinter.messagebox.showinfo("Cleaner by Dhruv", "Scaning in no time!")
        # self.ScanRubbish()
    ...
    def MenuDelRubbish(self):
        result = tkinter.messagebox.askquestion("Cleaner by Dhruv",
                                                "Deleting junk files may cost much time. Continue?")
        if result == 'no':
            return
        tkinter.messagebox.showinfo("Cleaner by Dhruv", "Deleting in no time!")
        self.drives = GetDrives()
    ...
    def MenuSearchFile(self):
        ...
        if result == 'no':
            return
        tkinter.messagebox.showinfo("Cleaner by Dhruv", "Searching in no time!")
...
#Credits to
# https://codingshiksha.com/python/python-3-tkinter-script-to-scan-system-and-cleanup-disk-and-drives-and-delete-trash-files-gui-desktop-app-full-project-for-beginners/
#Bye guys ill upload this in github!!
